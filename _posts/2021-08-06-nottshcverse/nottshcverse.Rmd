...
description: |
  ***TLTR:*** 
  ...
  All our functions are grouped into R packages because this made it easier for us to: *(i)* write good documentation of our code and analytical tasks, *(ii)* formally test our code, *(iii)* easily distribute updates across all team members, and *(iv)* integrate common data manipulations (or analyses) into interactive dashboards in a modular way.
author:
...
```{r fig-nottshcverse-overview-funs, echo=FALSE, fig.cap="Main goals that guided the development of the packages that are part of the {nottshcverse}."}
knitr::include_graphics(here::here("_posts/2021-08-06-nottshcverse/nottshcverse_overview_functions.png"))
```

1. `r emo::ji("thinking")` It's really hard to get the clinical data that is needed in a reproducible way that is consistent across different people who work on the same (or related) analysis. This is particularly true because most of the data is stored on SQL servers and only starts to make sense after joining multiple different datasets. `r emo::ji("nerd")` **lalala**
1. `r emo::ji("thinking")` Most of the time, the data is not in the format that is needed for further analyses. There may be specific data manipulations that are needed to make sense of the data and analyses it properly. `r emo::ji("nerd")` **lalala**
1. `r emo::ji("thinking")` The methods of data analyses and visualisations should be understandable, reproducible, and available to other people. Unfortunately this is not the case for most of the work that I've seen because the software tools that are used (e.g., Excel, PowerBI, and other silly solutions that are not worth their money) are not script based and usually shared in private emails. `r emo::ji("nerd")` **lalala**
1. `r emo::ji("thinking")` Code should be really well documented so that it is easy to understand what's going on. This includes the current version of the code as well as all previous versions and changes. This can be done using tools like Git and GitHub, but unfortunately most code is currently shared undocumented in private emails. `r emo::ji("nerd")` **lalala**
1. `r emo::ji("thinking")` Mistakes happen! Sometimes things that you have no control over can change and break your code that previously worked fine (e.g., the format of the raw data or a functions that someone else wrote). Therefore, it's important to continuously test whether the code is still working the way it's supposed to work. `r emo::ji("nerd")` **lalala**
