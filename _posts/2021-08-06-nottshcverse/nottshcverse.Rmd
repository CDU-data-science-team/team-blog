...
description: |
  ***TLTR:*** 
  Our goal was to make it easier to work with healthcare data that is stored in databases in a reproducible and collaborative way.
  We wrote lots of R functions for recurring analytical tasks that get automatically translated into SQL code and grouped these functions into R packages.
  This allowed us to communicate with large databases through R and make use of many advantages that cannot be done easily using other approaches, such as *(i)* good documentation of code and analytical tasks, *(ii)* formal testing of code, *(iii)* easy distribution of updates across across all team members, and *(iv)* modular integration of common data manipulations (or analyses) in interactive dashboards.
author:
...
There are many challenges when working with healthcare data, here is only a small selection of challenges that I think underlay most analyses.
Figure \@ref(fig:fig-nottshcverse-overview-funs) shows our solutions to these challenges.

1. It's really hard to get the clinical data that is needed in a reproducible way that is consistent across different people who work on the same (or related) analyses. This is particularly true because most of the data is stored on SQL servers and only starts to make sense after joining multiple different datasets.
1. Most of the time, the data is not in the format that is needed for further analyses. There may be specific data manipulations that are needed to make sense of the data and analyses it properly. 
1. The methods of data analyses and visualisations should be understandable, reproducible, and available to other people. Unfortunately this is not the case for most of the work that I've seen because the software tools that are used (e.g., Excel, PowerBI, and other silly solutions that are not worth their money) are not script based and usually shared in private emails.
1. Code should be really well documented so that it is easy to understand what's going on. This includes the current version of the code as well as all previous versions and changes. This can be done using tools like Git and GitHub, but unfortunately most code is currently shared undocumented in private emails.
1. Mistakes happen! Sometimes things that you have no control over can change and break your code that previously worked fine (e.g., the format of the raw data or a functions that someone else wrote). Therefore, it's important to continuously test whether the code is still working the way it's supposed to work.



```{r fig-nottshcverse-overview-funs, echo=FALSE, fig.cap="Main goals that guided the development the packages that are part of the {nottshcverse}."}
...
```{r fig-nottshcverse-overview-pkgs, echo=FALSE, fig.cap="Overview of some R packages developed by the Clinical Development Unit Data Science Team and the NHS-R Community."}
...
Here's a simple example to demonstrate how this can be done.
...
- **DOCUMENT** EVERYTHING! ABSOLUTELY EVERYTHING! MESSY DATA and TIDY DATA! MESSY CODE and TIDY CODE EVERY FUNCTION! EVERY SINGLE CHANGE!
...
- FIND THE LINK TO THE BLOG POST I SAW ON TWITTER
